...
function Alerts({ go, goVehicle }: { go: (s: Section) => void; goVehicle: (plate: VehicleKey) => void }) { return <><PageIntro kicker="05 / HUMAN REVIEW QUEUE" title="Alerts" copy="Prioritized events from the network. An alert is a lead, never a verdict." action={<div className="alert-count"><span className="dot red" /> 3 active events</div>} /><div className="alert-tabs"><button className="active">All alerts <b>3</b></button><button>Watchlist matches <b>2</b></button><button>Potential route anomalies <b>1</b></button></div><div className="alert-cards">{alerts.map((a, i) => <Card className={`full-alert ${a.tone}`} key={a.plate + a.time}><div className={`big-alert-icon ${a.tone}`}>{a.tone === 'critical' ? <Siren size={20} /> : <AlertTriangle size={20} />}</div><div className="alert-main"><div className="alert-title"><Badge tone={a.tone === 'critical' ? 'critical' : 'warn'}>{a.type}</Badge><span>{a.time} · {a.camera}</span></div><h3>{a.plate}</h3><p>{a.detail || `${a.place} · Watchlist identity observed with high confidence.`}</p><div className="alert-meta"><span><MapPin size={14} /> {a.place}</span><span><Clock3 size={14} /> {a.time}</span><Confidence value={a.confidence} /></div></div><button className="dark-button" onClick={() => goVehicle(a.plate as VehicleKey)}>{a.tone === 'critical' ? 'View trajectory' : 'Review trajectory'} <ArrowRight size={15} /></button></Card>)}</div><div className="review-note"><Info size={17} /><div><strong>Human review stays in the loop</strong><p>Route anomalies flag spatial-temporal inconsistencies for an operator to investigate. They do not indicate criminal activity on their own.</p></div></div></> }

export default function Dashboard() { const [active, setActive] = useState<Section>('Overview'); const [camera, setCamera] = useState(0); const [selected, setSelected] = useState<VehicleKey>('TN01AB1234'); const [navOpen, setNavOpen] = useState(false); const go = (s: Section) => setActive(s); const goVehicle = (plate: VehicleKey) => { setSelected(plate); setActive('Vehicle Tracking') }; return <div className="app-shell"><SideNav active={active} onSelect={go} open={navOpen} onClose={() => setNavOpen(false)} /><main className="main"><Header section={active} onMenu={() => setNavOpen(true)} /><div className="content">{active === 'Overview' && <Overview go={go} />}{active === 'ANPR Monitor' && <ANPR camera={camera} setCamera={setCamera} />}{active === 'Vehicle Tracking' && <Tracking selected={selected} setSelected={setSelected} />}{active === 'Traffic Analytics' && <Analytics />}{active === 'Alerts' && <Alerts go={go} goVehicle={goVehicle} />}</div><footer className="footer"><span><span className="tiny-dot" /> All systems operational</span><span>TrackCam frontend prototype · Smart India Hackathon 2026</span></footer></main></div> }
